...
const DriverMap = ({ busId = "bus-1", className = "", fullScreen = false }: DriverMapProps) => {
  ...
  const allPositions: [number, number][] = [
    ...bus.waypoints,
    ...busStudents.map((s) => [s.lat, s.lng] as [number, number]),
  ...
  const calcETA = (targetLat: number, targetLng: number): number => {
    const dist =
      Math.sqrt(
        Math.pow(busPos[0] - targetLat, 2) + Math.pow(busPos[1] - targetLng, 2)
      ) * 111;
    return Math.max(1, Math.round((dist / bus.speed) * 60));
  };

  return (
    <div className={`relative overflow-hidden rounded-xl ${fullScreen ? "h-full" : "h-[460px]"} ${className}`}>
      {routeLoading && (
      ...
      <MapContainer center={[31.1070, 30.9440]} zoom={14} style={{ height: "100%", width: "100%" }}>
        ...
        {traveled.length > 1 && (
          <Polyline
            positions={traveled}
            pathOptions={{ color: "#9ca3af", weight: 5, opacity: 0.7, dashArray: "10,6" }}
          />
        )}
        {remaining.length > 1 && (
          <Polyline
            positions={remaining}
            pathOptions={{ color: bus.colorHex, weight: 6, opacity: 0.9 }}
          />
        )}
        ...
        <Marker position={busPos} icon={createBusIcon(bus.colorHex)}>
          <Popup>
            <div style={{ minWidth: 140 }}>
              ...
              Students: {busStudents.length}<br />
        ...
        <Marker position={[bus.startLat, bus.startLng]} icon={startIcon}>
          <Popup><strong>🏁 Start Point</strong><br />{bus.number} departure</Popup>
        </Marker>
        ...
        {busStudents.map((student, idx) => (
          <Marker
            ...
            icon={createStudentPickupIcon(idx, bus.colorHex)}
          >
            <Popup>
              <div style={{ minWidth: 150 }}>
                <strong>👤 {student.name}</strong><br />
                📍 {student.locationName || "Pickup Point"}<br />
                🚌 Bus: {bus.number}<br />
        ...
        <Marker position={[UNIVERSITY.lat, UNIVERSITY.lng]} icon={universityIcon}>
          <Popup><strong>🏛️ {UNIVERSITY.name}</strong><br />Final Destination</Popup>
        </Marker>
