...
import { Bus, Users, ChevronRight, Search, AlertCircle, MapPin, Zap } from "lucide-react";
          ...
          {isSidebarOpen && (
            <motion.div initial={{ x: 300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 300, opacity: 0 }} className="w-80 border-l border-border/50 bg-background flex flex-col">
              <div className="p-4 border-b border-border/50 flex justify-between items-center">
                <h3 className="font-bold flex items-center gap-2"><Bus className="h-4 w-4 text-primary" /> Live Buses</h3>
              ...
              <div className="p-3">
                <div className="relative">
                  ...
                  <Input placeholder="Search buses or drivers..." className="pl-7 text-xs" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                </div>
              </div>

              <div className="px-3 pb-2">
                <div className="rounded-lg bg-secondary/20 p-2 space-y-1">
                  <p className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider mb-2">Legend</p>
                  {MOCK_BUSES.map((bus) => (
                    <div key={bus.id} className="flex items-center gap-2 text-xs">
                      ...
                      <span className="text-muted-foreground">— {bus.driverName}</span>
                    </div>
                  ))}
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3">
                {filteredBuses.length > 0 ? filteredBuses.map((bus) => {
                  ...
                  return (
                    <div key={bus.id} className="p-3 rounded-xl border bg-secondary/10 hover:bg-secondary/20 transition cursor-pointer" style={{ borderColor: bus.colorHex + "55", borderLeftWidth: 3, borderLeftColor: bus.colorHex }}>
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <Badge className="text-xs text-white border-none" style={{ background: bus.colorHex }}>Active</Badge>
                      </div>
                      ...
                      <div className="mt-2 space-y-0.5">
                        {assignedStudents.map((s) => (
                          <div key={s.id} className="flex items-center gap-1 text-[10px]">
                            <MapPin className="h-2.5 w-2.5" style={{ color: bus.colorHex }} />
                            <span>{s.name}</span>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="text-center py-10 text-muted-foreground text-xs"><AlertCircle className="h-6 w-6 mx-auto mb-2 opacity-20" />No buses found</div>
                )}
              </div>
