    ...
    <AppLayout title="Live Tracking Map">
      <div className="flex h-[calc(100vh-120px)] w-full rounded-xl overflow-hidden border border-border/50 bg-background relative">
        ...
        <div className="flex-1 relative">
          <AdminMap fullScreen />
          {!isSidebarOpen && (
            <Button variant="secondary" size="icon" className="absolute top-4 right-4 z-10 shadow-lg" onClick={() => setIsSidebarOpen(true)}>
            ...
            >
              ...
              <div className="p-4 border-t border-border/50 text-xs space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">{MOCK_BUSES.length}</span>
                </div>
                <div className="flex justify-between">
                  <span>Total Students</span>
                  <span className="font-bold">{MOCK_STUDENTS.length}</span>
                </div>
              </div>
            </motion.div>
          )}
